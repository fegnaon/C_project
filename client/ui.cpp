...
{   
    ...
    int ifsingle = 0;
    if(!strcmp(table.player1,"电脑")){
        outtextxy(550,350,"返回");
        outtextxy(550,300,"悔棋");
        ifsingle = 1;
    }
    ...
    if (table.turn != myturn){
        ...
        while(1)
        {
            ...
            if ((int)msg.is_left() == 1 && (int)msg.is_down() == 1){
                row = (msg.y-45)/30;
                column = (msg.x-45)/30;
                if (ifsingle == 1&&msg.x>=550&&msg.x<=600&&msg.y>=350&&msg.y<=375){//返回菜单
                    *i = 20;
                    return;
                }
                if (ifsingle == 1&&msg.x>=550&&msg.x<=600&&msg.y>=300&&msg.y<=325){//悔棋
                    *i = 19;
                    return;
                }
